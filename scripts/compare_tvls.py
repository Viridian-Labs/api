...
def dexscreener_request(address):
    ...
    if pairs is None or not isinstance(pairs, list) or len(pairs) == 0:
        return 0
    # print(pairs[0])
    tvl = pairs[0].get("liquidity").get("usd") if pairs[0].get("liquidity") else 0
    return float(tvl)
...
df = pd.DataFrame(diff_tvls, columns=["Address", "Symbol", "TVL OLD", "TVL NEW"])
df['Difference'] = df['TVL OLD'] - df['TVL NEW']
df.sort_values(by=["Difference"], inplace=True, ascending=False)

print(df)
print("TVLs with same values:")
df2 = pd.DataFrame(same_tvls, columns=["Address", "Symbol", "TVL OLD", "TVL NEW"])
print(df2)
print("TVLs with close values:")
df3 = pd.DataFrame(close_tvls, columns=["Address", "Symbol", "TVL OLD", "TVL NEW"])
print(df3)

print ("DEXSCREENER ANALYSIS")
print("TVLs with different values:")
df = pd.DataFrame(diff_tvls_dex, columns=["Address", "Symbol", "TVL NEW", "TVL DEX"])
df['Difference'] = df['TVL NEW'] - df['TVL DEX']
df.sort_values(by=["Difference"], inplace=True, ascending=False)
print(df)
print("TVLs with same values:")
df2 = pd.DataFrame(same_tvls_dex, columns=["Address", "Symbol", "TVL NEW", "TVL DEX"])
print(df2)
print("TVLs with close values:")
df3 = pd.DataFrame(close_tvls_dex, columns=["Address", "Symbol", "TVL NEW", "TVL DEX"])
print(df3)