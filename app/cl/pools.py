...
def _fetch_pools(debug):
    ...
    for pool_address, pool in pools.items():
        ...
        for day in valid_days:
            ...
            pool['projectedFees']['tokens'][pool['token0']['id']] += int(float(day['volumeToken0']) * int(pool['feeTier']) / 1e6 * 10**token0['decimals'] * fee_distribution['veVara'])
            pool['projectedFees']['tokens'][pool['token1']['id']] += int(float(day['volumeToken1']) * int(pool['feeTier']) / 1e6 * 10**token1['decimals'] * fee_distribution['veVara'])
    ...
    for pool_address, pool in pools.items():
        ...
        if pool['totalVeShareByPeriod'] > 0:
            ...
            pool['voteApr'] = totalUSD / 7 * 36500 / (pool['totalVeShareByPeriod'] * tokens[NATIVE_TOKEN_ADDRESS]['price'] / 1e18)
            ...
            if (pool['projectedFees']['days'] > 0):
                ...
                pool['projectedFees']['apr'] = projected_fees_usd / pool['projectedFees']['days'] * 36500 / (pool['totalVeShareByPeriod'] * tokens[NATIVE_TOKEN_ADDRESS]['price'] / 1e18)
        else:
            pool['voteApr'] = 0
